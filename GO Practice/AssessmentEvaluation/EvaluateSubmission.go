...
func getAssessmentSubmissions(w http.ResponseWriter,r *http.Request){
  go handleAssessmentSubmission(r.Body)

}

func handleAssessmentSubmission(submissions []AssessmetSubmit){
  var assessments []Assessment
  var assessmentsubmits []AssessmentSubmit
  var id SubmitId
  var schoolid string
  json.NewDecoder(submissions).Decode(&assessmentSubmits)
	schoolid=assessmentsubmits[0].SchoolId
	//Get the corresponding Assessments
	postid:=assessmentsubmits[0].AssessmentPost
	response,_:=http.Get("http://192.168.0.8:3131/getassessments/"+postid)
	...
	var submisssionpost SubmissionPost
	var attempts int=0
	...
	notanswered,correct,score:=evaluateSubmissions(assessments,assessmentsubmissions)

	//Here we can pass the structure as a aparametetr instead of passing 6 individual parameters
	submissionpost=prepareSubmissionPost(assessmentsubmits[0].AssessmentPost,assessmentsubmits[0].User,notanswered,correct,score,attempts)

  submissionpostjson,_:=json.Marshal(submissionpost)
	var url string="http://192.168.0.8:3131/savesubmissionpost/"+flag+"/"+submissionpostid
	fmt.Println(url)
	...
	_,err=http.Post("http://192.168.0.8:3131/savesubmissions","application/json",bytes.NewBuffer(jsondata))
	if err!=nil{
...
func getAttempts(userid string,postid string) int{
	...
	var url="http://192.168.0.8:3131/getassessmentpost/"+userid+"/"+postid
	res,_:=http.Get(url)
...
func prepareSubmissionPost(assessmentpost string,user string,notanswered,correct,score,attempts int) SubmissionPost{
...
func evaluateSubmissions(assessments []Assessment,assessmentsubmits []AssessmentSubmit)(int,int,int){
